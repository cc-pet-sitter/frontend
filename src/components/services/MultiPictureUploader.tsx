            ...
            for (let i = 0; i < filesArray.length; i++) {
                ...
                await new Promise<void>((resolve, reject) => {
                    uploadTask.on(
                        "state_changed",
                        (snapshot) => {
                            setUploadProgress(Math.round(((i + snapshot.bytesTransferred / snapshot.totalBytes) / filesArray.length) * 100));
                        },
                        (error) => {
    ...
    return (
        <div className="mt-4">
            ...
            {isUploading && uploadProgress > 0 && (
                ...
                <p className="mt-1 text-sm text-brown">{uploadProgress.toFixed(0)}%</p>
                </div>
            )}
        </div>
    );
